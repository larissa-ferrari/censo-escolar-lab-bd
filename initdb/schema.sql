...
ALTER TABLE
    usuario
MODIFY
    COLUMN nome VARCHAR(255) NOT NULL;

ALTER TABLE
    usuario
ADD
    COLUMN administrador BOOLEAN DEFAULT FALSE;

-- CRIAR PKS
ALTER TABLE
    escola
ADD
    CONSTRAINT PRIMARY KEY (CO_ENTIDADE);
...
ALTER TABLE
    matricula
ADD
    CONSTRAINT PRIMARY KEY (ID_MATRICULA);
...
ALTER TABLE
    ideb
ADD
    CONSTRAINT FOREIGN KEY (CO_ENTIDADE) REFERENCES escola(CO_ENTIDADE);

-- AULA 21/09
CREATE VIEW Escolas_Ativas AS
SELECT
    *
FROM
    escola
...
CREATE VIEW Colunas_Usadas_Escola AS
SELECT
    NO_ENTIDADE,
    CO_ENTIDADE,
...
CREATE VIEW Colunas_Usadas_Matricula AS
SELECT
    ID_MATRICULA,
    NU_IDADE
FROM
...
CREATE VIEW Colunas_Usadas_Turma AS
SELECT
    NU_MATRICULAS,
    NO_TURMA
FROM
...
CREATE VIEW Colunas_Usadas_Docente AS
SELECT
    CO_PESSOA_FISICA,
    NU_IDADE_REFERENCIA,
    TP_SEXO
...
DELIMITER $ $ CREATE FUNCTION Dias_Letivos(
    DT_ANO_LETIVO_INICIO DATE,
    DT_ANO_LETIVO_TERMINO DATE
) RETURNS INT DETERMINISTIC BEGIN RETURN DATEDIFF(DT_ANO_LETIVO_TERMINO, DT_ANO_LETIVO_INICIO);

END $ $ DELIMITER;

DELIMITER $ $ CREATE FUNCTION Validar_Email(email VARCHAR(255)) RETURNS BOOLEAN DETERMINISTIC BEGIN IF email REGEXP '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN RETURN TRUE;
